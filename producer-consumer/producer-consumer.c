...
int pcq_create(pc_queue_t *queue, size_t capacity) {
	...
	queue->pcq_head = 0; //FIXME: check that an empty queue is never accessed
...
int pcq_enqueue(pc_queue_t *queue, void *elem) {
	...
	if (queue->pcq_head+1 == queue->pcq_capacity) queue->pcq_head = 0;
	else queue->pcq_head++;
...
void *pcq_dequeue(pc_queue_t *queue) {
	...
	if (queue->pcq_tail+1 == queue->pcq_capacity) queue->pcq_tail = 0;
	else queue->pcq_tail++;
	pthread_mutex_unlock(&queue->pcq_head_lock);
